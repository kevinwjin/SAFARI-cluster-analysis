## Cluster analysis of SAFARI-extracted shape features from map shapes
## Authors: Qiwei Li, Kevin Jin

##### CREATE NORMALIZED MAP SHAPES FROM RAW MAP DATA #####
...
extract_features <- function(img) {
  ...
  if (img == "vatican.png") {
    img_segs <- binary.segmentation(this_img,
                                    id = c("maps"), # Placeholder IDs
                                    filter = 1, # Noise removal of anything smaller than 150 pixels;
    ...
    img_segs <- binary.segmentation(this_img,
                                    id = c("maps"), # Placeholder IDs
                                    filter = 5, # Noise removal of anything smaller than 150 pixels;
                                    # Be careful because largest shape may be smaller
                                    # than 150 for some images;
                                    # Always choose the largest filter per image
                                    k = 3, # Enlarge shape by a factor of 3 to avoid 0 thickness and revisiting
...
cl <- makeCluster(detectCores() - 1) # Allocate number of cores as detected
